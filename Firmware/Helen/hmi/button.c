...
#define HMI_CONFIG_LOG_ENABLED 1 /// TODO: just temporary until project specific sdk_config
#define HMI_CONFIG_LOG_LEVEL   3

#define NRF_LOG_MODULE_NAME hmi
#if HMI_CONFIG_LOG_ENABLED
#define NRF_LOG_LEVEL HMI_CONFIG_LOG_LEVEL
...
#define TIMEBASE                APP_TIMER_TICKS(10) // debouncing and evaluating timebase
...
static void setState(buttonState_t newState)
{
    if (state == newState)
    {
        NRF_LOG_WARNING("already in requested state")
        return;
    }
    if (newState == STATE_DEBOUNCEANDEVALUATE)
    {
        ...
        for (uint_fast8_t i = 0; i < ARRAY_SIZE(debounce); i++)
        {
            if (debounce[i].pin != FREE_PIN)
                nrfx_gpiote_in_event_disable(debounce[i].pin);
        }
        // start timer
        ret_code_t errCode = app_timer_start(butTimer, TIMEBASE, NULL);
        LOG_ERROR_CHECK("starting timer error %d", errCode);

        NRF_LOG_INFO("debouncing and evaluating");
    }
    else
    {
        // stop timer
        ret_code_t errCode = app_timer_stop(butTimer);
        LOG_ERROR_CHECK("stopping timer error %d", errCode);

        // enable pin change interrupts
        for (uint_fast8_t i = 0; i < ARRAY_SIZE(debounce); i++)
        {
            if (debounce[i].pin != FREE_PIN)
                nrfx_gpiote_in_event_enable(debounce[i].pin, true);
        }
        NRF_LOG_INFO("waiting for pin change");
    }
...
static void pinChangeHandler(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
    ...
    {
        NRF_LOG_WARNING("pin change for unknown pin received");
        return;
    }
...
static ret_code_t init()
{
    ...
    if (!nrfx_gpiote_is_init())
    {
        ...
        {
            NRF_LOG_ERROR("GPIOTE init failed with error %d.", errCode);
            return errCode;
        }
    }
    ...
    if (errCode != NRF_SUCCESS)
    {
        NRF_LOG_ERROR("[BRD]: timer creation error %d.", errCode);
        return errCode;
    }
#endif
...
ret_code_t but_DebounceInit(but_debounceInit_t const* pInit)
{
    ...
    nrfx_gpiote_in_config_t buttonConfig = NRFX_GPIOTE_CONFIG_IN_SENSE_TOGGLE(false);
    buttonConfig.pull = pInit->polarity == BUTTON_ACTIVE_LOW ? NRF_GPIO_PIN_PULLUP : NRF_GPIO_PIN_PULLDOWN;
    errCode = nrfx_gpiote_in_init(pInit->pinNo, &buttonConfig, &pinChangeHandler);
    if (errCode != NRF_SUCCESS)
    {
        NRF_LOG_ERROR("GPIOTE input pin init failed with error %d.", errCode);
        return errCode;
    }
    nrfx_gpiote_in_event_enable(pInit->pinNo, true);
