...
/* Private defines -----------------------------------------------------------*/
#define TIMEBASE                APP_TIMER_TICKS(500)

#define DELETE_TIMEOUT          (APP_TIMER_TICKS(3000)/TIMEBASE)
...
uint32_t hmi_Init(hmi_init_t const* pInit)
{
    ...
    if (errCode != NRF_SUCCESS)
        NRF_LOG_ERROR("Timer creation failed.");
...
void hmi_SetLed(hmi_ledType_t led, hmi_ledState_t state)
{
    ...
    if (timerNeeded != isAlreadyActive)
    {
        ...
        if (!timerNeeded)
            {NRF_LOG_INFO("LED interface disabled.");}
        else
            {NRF_LOG_INFO("LED interface enabled.");}
    }
