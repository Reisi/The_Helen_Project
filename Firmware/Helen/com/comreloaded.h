/**
  ...
  *          rear light.
  *          Receiving messages is done withing a timer interrupt handler. Due
  *          to the fact that the softdevice might interrupt the code execution
  *          on higher interrupt levels, reception error can occur.
  *          Transmitting messages is normaly also done in the timer interrupt
  *          handler, but can optionally also be done by misusing a SPIM
  *          interface. In this case assign an unused high-speed capable pin to
  *          the dummy pin in the init structure, because the SPIM interface can
  *          not be operated without a clock pin.
  ******************************************************************************
  */
...
typedef struct
{
    ...
    uint32_t dummyPin;  // pin used for SPIM clk, COM_PIN_NOT_USED if timer should be used for transmitting
} com_init_t;
