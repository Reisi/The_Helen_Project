...
#define VALID_TIMEOUT   APP_TIMER_TICKS(30000)
...
static void onStatusReceived(com_message_t const* pMsg)
{
        ...
        pMsg->len == MSG_LEN_TAIL)
    {
        ...
        taillight.timestamp = app_timer_cnt_get();
    }
        ...
        pMsg->len == MSG_LEN_BAT)
    {
        ...
        battery.timestamp = app_timer_cnt_get();
    }
}
...
ret_code_t cmh_GetTaillightPower(q3_13_t* pPower)
{
    ...
    *pPower = taillight.power;
    ...
    else if (app_timer_cnt_diff_compute(time, taillight.timestamp) < VALID_TIMEOUT && taillight.timestamp != 0)
...
ret_code_t cmh_GetBatterySOC(q8_t* pSOCinPercent)
{
    ...
    *pSOCinPercent = battery.soc;
    ...
    if (time == 0)
        retVal = NRF_ERROR_INVALID_STATE;   // timer inactive
    else if (app_timer_cnt_diff_compute(time, battery.timestamp) < VALID_TIMEOUT && battery.timestamp != 0)
        retVal = NRF_SUCCESS;               // data valid
    else
    {
