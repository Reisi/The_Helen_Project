...
#define UTIL_CONFIG_LOG_ENABLED 1 /// TODO: just temporary until project specific sdk_config
#define UTIL_CONFIG_LOG_LEVEL   3

#define NRF_LOG_MODULE_NAME util
#if UTIL_CONFIG_LOG_ENABLED
#define NRF_LOG_LEVEL UTIL_CONFIG_LOG_LEVEL
...
static void fdsEventHandler(fds_evt_t const* pEvt)
{
    ...
    {
        LOG_ERROR_CHECK("fds init error %d in event", errCode);
        initResult = errCode;
    }
...
static void init()
{
    ...
    if (errCode != NRF_SUCCESS)
    {
        NRF_LOG_ERROR("fds register error %d", errCode);
        initResult = errCode;
        return;
    }
    ...
    if (errCode != NRF_SUCCESS)
    {
        NRF_LOG_ERROR("fds init error %d", errCode);
        initResult = errCode;
        return;
    }
...
ret_code_t ds_Read(uint16_t fileId, uint16_t recordKey, void const** ppData, uint16_t* pLengthWords)
{
    ...
    if (errCode == NRF_SUCCESS)
    {
        ...
        *ppData = rec.p_data;
        *pLengthWords = rec.p_header->length_words;
...
ret_code_t ds_Write(uint16_t fileId, uint16_t recordKey, void const* pData, uint16_t lengthWords, ds_reportHandler_t pHandler)
{
    ...
    rec.file_id           = fileId;
    rec.key               = recordKey;
    rec.data.p_data       = pData;
    rec.data.length_words = lengthWords;
