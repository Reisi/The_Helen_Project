...
static uint32_t light_measurement_char_add(ble_lcs_t * p_lcs, const ble_lcs_init_t * p_lcs_init)
{
    ...
    init_value[BLE_LCS_LM_MIN_CHAR_LEN - 1] = 0;

    memset(&add_char_params, 0, sizeof(add_char_params));
    add_char_params.uuid              = BLE_UUID_LCS_LM_CHARACTERISTIC;
    add_char_params.uuid_type         = p_lcs->uuid_type;
    add_char_params.init_len          = BLE_LCS_LM_MIN_CHAR_LEN;
    add_char_params.max_len           = BLE_LCS_LM_MAX_CHAR_LEN;
    add_char_params.is_var_len        = true;
    add_char_params.p_init_value      = init_value;
    add_char_params.char_props.notify = 1;
    add_char_params.cccd_write_access = p_lcs_init->lcs_lf_rd_sec;

    return characteristic_add(p_lcs->service_handle, &add_char_params, &p_lcs->lm_handles);
...
static uint32_t light_feature_char_add(ble_lcs_t * p_lcs, const ble_lcs_init_t * p_lcs_init)
{
    ...
    memset(&add_char_params, 0, sizeof(add_char_params));
    add_char_params.uuid            = BLE_UUID_LCS_LF_CHARACTERISTIC;
    add_char_params.uuid_type       = p_lcs->uuid_type;
    ...
    add_char_params.read_access     = p_lcs_init->lcs_lf_rd_sec;

    return characteristic_add(p_lcs->service_handle, &add_char_params, &p_lcs->lf_handles);
...
static void on_connect(ble_lcs_t * p_lcs, ble_evt_t const * p_ble_evt)
{
    uint32_t                err_code;
    ble_lcs_evt_t           evt;
    ...
    if (p_client == NULL/* && p_lcs->error_handler != NULL*/)
    {
        /*p_lcs->error_handler(NRF_ERROR_NOT_FOUND);*/
        return;
    }
    p_client->conn_handle = p_ble_evt->evt.gap_evt.conn_handle;

    if (p_lcs->evt_handler)
    {
        return;
    }
    ...
    err_code = sd_ble_gatts_value_get(p_ble_evt->evt.gap_evt.conn_handle,
                                      p_lcs->ctrl_pt.lc_ctrlpt_handles.cccd_handle,
                                      &gatts_val);
    if (err_code == NRF_SUCCESS && ble_srv_is_indication_enabled(gatts_val.p_value))
    {
        p_client->is_lcp_indic_enabled = true;

        memset(&evt, 0, sizeof(ble_lcs_evt_t));
        evt.evt_type    = BLE_LCS_EVT_LCP_INDICATION_ENABLED;
        evt.p_lcs       = p_lcs;
        evt.conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
        evt.p_client    = p_client;
...
static void on_disconnect(ble_lcs_t * p_lcs, ble_evt_t const * p_ble_evt)
{
    ble_lcs_evt_t evt;
    ble_lcs_client_spec_t * p_client;
    ...
    if (p_lcs->evt_handler != NULL)
    {
        ...
        if (p_client->is_lcp_indic_enabled)
        {
            p_client->is_lcp_indic_enabled = false;
            memset(&evt, 0, sizeof(ble_lcs_evt_t));
            evt.evt_type    = BLE_LCS_EVT_LCP_INDICATION_DISABLED;
            evt.p_lcs       = p_lcs;
            evt.conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
            evt.p_client    = p_client;
...
static void on_lm_cccd_write(ble_lcs_t * p_lcs, ble_gatts_evt_write_t const * p_evt_write, uint16_t conn_handle)
{
    ble_lcs_evt_t evt;
    ble_lcs_client_spec_t * p_client;
...
static void on_lcp_cccd_write(ble_lcs_t * p_lcs, ble_gatts_evt_write_t const * p_evt_write, uint16_t conn_handle)
{
    ble_lcs_evt_t evt;
    ble_lcs_client_spec_t * p_client;
...
static void on_write(ble_lcs_t * p_lcs, ble_evt_t const * p_ble_evt)
{
    ...
    else if (p_evt_write->handle == p_lcs->ctrl_pt.lc_ctrlpt_handles.cccd_handle)
    {
        on_lcp_cccd_write(p_lcs, p_evt_write, p_ble_evt->evt.gatts_evt.conn_handle);
    }
}
...
uint32_t ble_lcs_init(ble_lcs_t * p_lcs, uint8_t max_clients, const ble_lcs_init_t * p_lcs_init)
{
    ...
    ctrlpt_init.lc_lcp_cccd_wr_sec = p_lcs_init->lcs_lcp_cccd_wr_sec;
    ctrlpt_init.lc_lcp_wr_sec      = p_lcs_init->lcs_lcp_wr_sec;
...
void ble_lcs_on_ble_evt(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    ble_lcs_t * p_lcs = (ble_lcs_t *)p_context;
