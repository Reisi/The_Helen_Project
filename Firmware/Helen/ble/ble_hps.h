...
#endif // BLE_HPS_BLE_OBSERVER_PRIO
...
/**@brief Function prototype to add board specific characteristics to the Helen Project Service. */
typedef uint32_t (*ble_hps_brd_char_add_t)(uint8_t uuid_type, uint16_t service_handle, void * p_context);

typedef struct
{
    ble_hps_brd_char_add_t  brd_char_add;       /**< Function to add board specific characteristics. */
    void                  * p_context;          /**< context for board specific characteristics */
} ble_hps_brd_char_t;

#define BLE_HPS_BRD_CHAR_REGISTER(_name, _char, _context)                           \
    NRF_SECTION_ITEM_REGISTER(ble_hps_brd_char, static ble_hps_brd_char_t _name) =  \
    {                                                                               \
        .brd_char_add = _char,                                                      \
        .p_context    = _context,                                                   \
    }
...
//typedef struct
//{
//    ble_user_mem_block_t buffer;/**< the buffer used for long writes */
//    uint16_t  conn_handle;      /**< the connection the buffer is allocated to */
//} ble_hps_modes_buf_t;
 ...
 */
void ble_hps_on_ble_evt(ble_evt_t const * p_ble_evt, void * p_context);

/** @brief Function for handling the events from the queued write module
 *
 * @param[in] p_hps
 * @param[in] p_qwr
 * @param[in] p_evt
 * @return BLE_GATT_STATUS_SUCCESS or the return value of the hps event handler
 */
