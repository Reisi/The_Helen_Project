...
#define BLE_HPS_BRD_CHAR_CNT()      NRF_SECTION_ITEM_COUNT(ble_hps_brd_char, ble_hps_brd_char_t)
#define BLE_HPS_BRD_CHAR_GET(cnt)   NRF_SECTION_ITEM_GET(ble_hps_brd_char, ble_hps_brd_char_t, cnt)
...
static uint32_t modes_char_add(ble_hps_t * p_hps, ble_hps_init_t const * p_hps_init)
{
    ...
    // add handle to queued writes module if necessary
    if (p_hps_init->modes.total_size > 20 && p_hps_init->p_qwr_ctx != NULL)
    {
        err_code = nrf_ble_qwr_attr_register(p_hps_init->p_qwr_ctx, p_hps->modes_handles.value_handle);
    }
...
static void on_connect(ble_hps_t * p_hps, ble_evt_t const * p_ble_evt)
{
    ...
    // check the hosts measuremnt CCCD value to inform the application if it has to send notifications
...
static void on_rw_authorize_request(ble_hps_t * p_hps, ble_evt_t const * p_ble_evt)//(ble_KD2_t * p_KD2, ble_gatts_evt_t const * p_gatts_evt)
{
    ble_gatts_evt_rw_authorize_request_t const * p_auth_req = &p_ble_evt->evt.gatts_evt.params.authorize_request;

    if (p_auth_req->type == BLE_GATTS_AUTHORIZE_TYPE_WRITE)
    {
        if ((p_auth_req->request.write.op == BLE_GATTS_OP_WRITE_REQ) ||
...
void ble_hps_on_ble_evt(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    ble_hps_t * p_hps = (ble_hps_t *)p_context;
