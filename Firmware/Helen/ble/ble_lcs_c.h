...
typedef enum
{
    BLE_LCS_C_ERROR_EVT_GATT_WRITE, /**< Event indicating that gatt queue moduel has returned an error. */
...
/**@brief   Structure containing the handles related to the Light Control Service found on the peer. */
typedef struct
{
...
typedef struct
{
    ...
    ble_lcs_c_db_t          lcs_db;             /**< Handles related to LCS on the peer*/
...
typedef struct
{
    ...
    union
    {
        uint16_t conn_handle;               /**< This is only filled if the evt_type is @ref BLE_LCS_C_EVT_DISCOVERY_COMPLETE. */
        ble_lcs_c_server_spec_t * p_server; /**< pointer to server data this event is related to */
    };
    union
    {
        ble_lcs_c_db_t        lcs_db;       /**< Light Control Service related handles found on the peer device. This is filled if the evt_type is @ref BLE_LCS_C_EVT_DISCOVERY_COMPLETE.*/
...
typedef struct
{
    ...
    union
    {
        ...
        uint32_t err_code;                  /**< error code of the gatt queue module @ref BLE_LCS_C_ERROR_EVT_GATT_WRITE. */
    } data;
...
static ble_lcs_c_server_spec_t _name ## _server_data[_lcs_max_servers]; \
static ble_lcs_c_t _name = { .p_server = _name ## _server_data,};       \
NRF_SDH_BLE_OBSERVER(_name ## _obs, BLE_LCS_C_BLE_OBSERVER_PRIO,        \
                     ble_lcs_c_on_ble_evt, &_name)
...
uint32_t ble_lcs_c_handles_assign(ble_lcs_c_t * p_ble_lcs_c, uint16_t conn_handle, ble_lcs_c_db_t * p_peer_handles);
