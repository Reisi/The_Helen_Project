...
#define WRITE_MESSAGE_LENGTH  20                  /**< The maximum size of a control point cmd message */
#define MAX_MODE_LIST_LENGTH  (WRITE_MESSAGE_LENGTH-2) /**< The maximum size of a mode list that can be sent */
#define CCCD_LENGTH           2



const ble_uuid128_t  ble_lcs_c_base_uuid = {{0x1B, 0xC5, 0xD5, 0xA5, 0x02, 0x00, 0x0e, 0x84, 0xe4, 0x11, 0x7d, 0xed, 0x00, 0x00, 0x77, 0x4f}};
...
{
    ble_lcs_c_t * p_ble_lcs_c = (ble_lcs_c_t *)p_ctx;

    NRF_LOG_DEBUG("A GATT Client error has occurred on conn_handle: 0X%X", conn_handle);

    ble_lcs_c_server_spec_t * p_server;

    p_server = get_server_data_by_conn_handle(BLE_CONN_HANDLE_INVALID, p_ble_lcs_c->p_server);
    if (p_server == NULL)
    {
        return;
    }

    if (p_ble_lcs_c->error_handler != NULL)
    {
        ble_lcs_c_error_evt_t evt;

        evt.evt_type = BLE_LCS_C_ERROR_EVT_GATT_WRITE;
        evt.p_server = p_server;
        evt.data.err_code = nrf_error;
 ...
 void ble_lcs_c_on_db_disc_evt(ble_lcs_c_t * p_ble_lcs_c, ble_db_discovery_evt_t const * p_evt)
{
    ...
    if (p_evt->evt_type == BLE_DB_DISCOVERY_COMPLETE &&
        p_evt->params.discovered_db.srv_uuid.uuid == BLE_UUID_LCS_SERVICE &&
        p_evt->params.discovered_db.srv_uuid.type == p_ble_lcs_c->uuid_type)
    {
        ...
        for (uint_fast8_t i = 0; i < p_evt->params.discovered_db.char_count; i++)
        {
                ...
                BLE_UUID_LCS_LM_CHARACTERISTIC)
            {
                ...
                evt.data.lcs_db.lcm_handle =
                    p_evt->params.discovered_db.charateristics[i].characteristic.handle_value;
                evt.data.lcs_db.lcm_cccd_handle =
        ...
        for (uint_fast8_t i = 0; i < p_evt->params.discovered_db.char_count; i++)
        {
            ...
            {
                // Found HID information characteristic. Store handle and break
                evt.data.lcs_db.lccp_handle =
                    p_evt->params.discovered_db.charateristics[i].characteristic.handle_value;
                evt.data.lcs_db.lccp_cccd_handle =
                    p_evt->params.discovered_db.charateristics[i].cccd_handle;
                break;
            }
        }
        ...
        p_server = get_server_data_by_conn_handle(p_evt->conn_handle, p_ble_lcs_c->p_server);
        if (p_server != NULL)
        {
            p_server->lcs_db = evt.data.lcs_db;
        }
...
static void on_disconnect(ble_lcs_c_t * p_ble_lcs_c, const ble_evt_t * p_ble_evt)
{
    ...
    p_server = get_server_data_by_conn_handle(p_ble_evt->evt.gap_evt.conn_handle, p_ble_lcs_c->p_server);
    if (p_server == NULL)
    {
    ...
    p_server->lcs_db.lcm_handle       = BLE_GATT_HANDLE_INVALID;
    p_server->lcs_db.lcm_cccd_handle  = BLE_GATT_HANDLE_INVALID;
    p_server->lcs_db.lcf_handle       = BLE_GATT_HANDLE_INVALID;
    p_server->lcs_db.lccp_handle      = BLE_GATT_HANDLE_INVALID;
    p_server->lcs_db.lccp_cccd_handle = BLE_GATT_HANDLE_INVALID;
}
...
static void on_hvx(ble_lcs_c_t * p_ble_lcs_c, const ble_evt_t * p_ble_evt)
{
    ...
    p_server = get_server_data_by_conn_handle(p_ble_evt->evt.gattc_evt.conn_handle, p_ble_lcs_c->p_server);
    if (p_server == NULL)
    {
        return; // device this event is related to has no light control service
    }
    ...
    if (p_ble_evt->evt.gattc_evt.params.hvx.handle == p_server->lcs_db.lcm_handle)
    {
    ...
    if (p_ble_evt->evt.gattc_evt.params.hvx.handle == p_server->lcs_db.lccp_handle)
    {
        uint32_t err_code;
        uint16_t data_size;

        // send confirmation
        err_code = sd_ble_gattc_hv_confirm(p_server->conn_handle, p_server->lcs_db.lccp_handle);
...
static void on_write_rsp(ble_lcs_c_t * p_ble_lcs_c, const ble_evt_t * p_ble_evt)
{
    ...
    if (p_ble_evt->evt.gattc_evt.gatt_status != BLE_GATT_STATUS_SUCCESS &&
        p_ble_lcs_c->error_handler != NULL)
    {
        ble_lcs_c_error_evt_t evt;
        ...
        if (p_ble_evt->evt.gattc_evt.params.write_rsp.handle == p_server->lcs_db.lcm_cccd_handle)
        {
            evt.evt_type = BLE_LCS_C_ERROR_EVT_LCM_NOTIFY;
        }
        else if (p_ble_evt->evt.gattc_evt.params.write_rsp.handle == p_server->lcs_db.lccp_handle)
        {
            evt.evt_type = BLE_LCS_C_ERROR_EVT_LCCP_WRITE;
        }
        else if (p_ble_evt->evt.gattc_evt.params.write_rsp.handle == p_server->lcs_db.lccp_cccd_handle)
        {
...
static void on_read_rsp(ble_lcs_c_t * p_ble_lcs_c, const ble_evt_t * p_ble_evt)
{
    ...
    if (p_response->handle == p_server->lcs_db.lcf_handle)
    {
        memset(&evt, 0, sizeof(evt));
 ...
 */
static uint32_t cccd_configure(ble_lcs_c_t * p_ble_lcs_c, uint16_t conn_handle, uint16_t handle_cccd, uint16_t cccd_val)
{
    nrf_ble_gq_req_t lcs_c_req;
    uint8_t cccd[CCCD_LENGTH];

    cccd[0] = LSB_16(cccd_val);
    cccd[1] = MSB_16(cccd_val);

    memset(&lcs_c_req, 0, sizeof(lcs_c_req));
    lcs_c_req.type                        = NRF_BLE_GQ_REQ_GATTC_WRITE;
    lcs_c_req.error_handler.cb            = gatt_error_handler;
    lcs_c_req.error_handler.p_ctx         = p_ble_lcs_c;
    lcs_c_req.params.gattc_write.handle   = handle_cccd;
    lcs_c_req.params.gattc_write.len      = CCCD_LENGTH;
    lcs_c_req.params.gattc_write.p_value  = cccd;
    lcs_c_req.params.gattc_write.offset   = 0;
    lcs_c_req.params.gattc_write.write_op = BLE_GATT_OP_WRITE_REQ;

    return nrf_ble_gq_item_add(p_ble_lcs_c->p_gatt_queue, &lcs_c_req, conn_handle);
...
uint32_t ble_lcs_c_init(ble_lcs_c_t * p_ble_lcs_c, uint8_t num_of_servers, ble_lcs_c_init_t * p_ble_lcs_c_init)
{
    ...
    while (num_of_servers)
    {
        ...
        p_ble_lcs_c->p_server[num_of_servers - 1].lcs_db.lcm_handle       = BLE_GATT_HANDLE_INVALID;
        p_ble_lcs_c->p_server[num_of_servers - 1].lcs_db.lcm_cccd_handle  = BLE_GATT_HANDLE_INVALID;
        p_ble_lcs_c->p_server[num_of_servers - 1].lcs_db.lcf_handle       = BLE_GATT_HANDLE_INVALID;
        p_ble_lcs_c->p_server[num_of_servers - 1].lcs_db.lccp_handle      = BLE_GATT_HANDLE_INVALID;
        p_ble_lcs_c->p_server[num_of_servers - 1].lcs_db.lccp_cccd_handle = BLE_GATT_HANDLE_INVALID;
        num_of_servers--;
    }

    p_ble_lcs_c = p_ble_lcs_c;
...
uint32_t ble_lcs_c_lcm_notif_enable(ble_lcs_c_t * p_ble_lcs_c, uint16_t conn_handle, bool enable)
{
    ...
    p_server = get_server_data_by_conn_handle(conn_handle, p_ble_lcs_c->p_server);
    if (p_server == NULL)
    {
    ...
    return cccd_configure(p_ble_lcs_c, conn_handle, p_server->lcs_db.lcm_cccd_handle, cccd_val);
}
...
uint32_t ble_lcs_c_feature_read(ble_lcs_c_t * p_ble_lcs_c, uint16_t conn_handle)
{
    ...
    memset(&lcs_c_req, 0, sizeof(lcs_c_req));
    lcs_c_req.type                     = NRF_BLE_GQ_REQ_GATTC_READ;
    lcs_c_req.error_handler.cb         = gatt_error_handler;
    lcs_c_req.error_handler.p_ctx      = p_ble_lcs_c;
    lcs_c_req.params.gattc_read.handle = p_server->lcs_db.lcf_handle;
...
uint32_t ble_lcs_c_cp_write(ble_lcs_c_t * p_ble_lcs_c, uint16_t conn_handle, const ble_lcs_c_cp_write_t* p_command)
{
    ...
    ble_lcs_c_server_spec_t * p_server;
    nrf_ble_gq_req_t lcs_c_req;
    uint8_t value[WRITE_MESSAGE_LENGTH];
    ...
    lcs_c_req.error_handler.cb            = gatt_error_handler;
    lcs_c_req.error_handler.p_ctx         = p_ble_lcs_c;
    lcs_c_req.params.gattc_write.handle   = p_server->lcs_db.lccp_handle;
    lcs_c_req.params.gattc_write.p_value  = value;
    lcs_c_req.params.gattc_write.offset   = 0;
    lcs_c_req.params.gattc_write.write_op = BLE_GATT_OP_WRITE_REQ;

    err_code = encode_control_point(p_command, value, &lcs_c_req.params.gattc_write.len);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_lcs_c_cp_indic_enable(ble_lcs_c_t * p_ble_lcs_c, uint16_t conn_handle, bool enable)
{
    ...
    return cccd_configure(p_ble_lcs_c, conn_handle, p_server->lcs_db.lccp_cccd_handle, cccd_val);
}

uint32_t ble_lcs_c_handles_assign(ble_lcs_c_t *    p_ble_lcs_c,
                                  uint16_t         conn_handle,
                                  ble_lcs_c_db_t * p_peer_handles)
{
    if (p_ble_lcs_c == NULL)
    {
        return NRF_ERROR_NULL;
    ...
    {
        p_server->lcs_db = *p_peer_handles;
    }
