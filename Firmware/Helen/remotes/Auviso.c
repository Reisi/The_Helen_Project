...
/*#if (NRF_SD_BLE_API_VERSION >= 6)
    uint16_t len = p_adv_data->data.len;
    uint8_t* p_data = p_adv_data->data.p_data;
#else
    uint16_t len = p_adv_data->dlen;
    uint8_t* p_data = p_adv_data->data;
#endif*/
    ...
    while (index < len)
    {
        ...
        {
            if (strncmp("SmartRemote", (char*)&p_data[index + 2], strlen("SmartRemote")) == 0)
                match_name = true;
        }
...
void rem_auviso_on_hids_c_evt(ble_hids_c_t * p_ble_hids_c, ble_hids_c_evt_t * p_evt)
{
    ...
    if (p_evt->evt_type != BLE_HIDS_C_EVT_REPORT_NOTIFICATION || p_evt->params.report.index != 2)
    {
        return;
    }
